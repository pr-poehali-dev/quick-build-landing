...
const P1_A =
  "https://cdn.poehali.dev/projects/571d06ae-01f7-46bc-a2c0-5e7834965168/bucket/d45a25a1-2591-498b-8337-ec1b3c7b61c0.jpg";
const P1_B =
  "https://cdn.poehali.dev/projects/571d06ae-01f7-46bc-a2c0-5e7834965168/bucket/c9f7cd62-bce5-4e2c-a9ee-289674a7df39.jpg";
const P1_C =
...
const STATS = [
  ...
  },
  {
    num: 150,
    suffix: "+",
    title: "Партнеров-строителей и проектировщиков",
    desc: "Сертифицированные проектно-строительные компании по всей России",
  },
];
...
const projects: Project[] = [
  {
    id: 1,
    photos: [P1_BG, P1_A, P1_B, P1_C],
    title: "Камера проращивания",
    dims: "11,8×23,8×4 м",
    area: "280 м²",
    locationShort: "Московская обл., д. Непейно",
    locationFull: "Россия, Московская обл., д. Непейно",
    purpose: "Сельскохозяйственные здания",
    details: ["Наличие кранов: нет", "Наличие антресолей: нет"],
    length: "23,8 м",
    width: "11,8 м",
    height: "4 м",
    category: "Сельскохозяйственные здания",
  },
  {
    id: 2,
        ...
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
          {p.photos.map((_, i) => (
            <button
              ...
              style={{
                background: imgIdx === i ? "#fff" : "rgba(255,255,255,0.35)",
              }}
            />
          ))}
        </div>
...
const FRAME_RATES: Record<number, Record<number, Record<number, number>>> = {
  3.6: {
    24: { 12: 2525, 18: 2572, 24: 1980 },
    30: { 12: 1796, 18: 1389, 24: 1488 },
    36: { 12: 1861, 18: 1008, 24: 1158 },
    42: { 12: 957, 18: 734, 24: 914 },
    48: { 12: 695, 18: 534, 24: 738 },
    54: { 12: 493, 18: 375, 24: 598 },
  },
  4.8: {
    24: { 12: 3214, 18: 2144, 24: 2550 },
    30: { 12: 2493, 18: 1576, 24: 1376 },
    36: { 12: 2020, 18: 1199, 24: 1070 },
    42: { 12: 1676, 18: 929, 24: 848 },
    48: { 12: 1420, 18: 731, 24: 679 },
    54: { 12: 1222, 18: 576, 24: 1224 },
  },
  6.0: {
    24: { 12: 3399, 18: 2681, 24: 2439 },
    30: { 12: 3014, 18: 1883, 24: 1648 },
    36: { 12: 2145, 18: 1485, 24: 1324 },
    42: { 12: 2155, 18: 1811, 24: 1734 },
    48: { 12: 1501, 18: 995, 24: 913 },
    54: { 12: 1307, 18: 832, 24: 781 },
  },
  7.2: {
    24: { 12: 4274, 18: 3841, 24: 3694 },
    30: { 12: 3466, 18: 2641, 24: 2354 },
    36: { 12: 2931, 18: 2212, 24: 1968 },
    42: { 12: 2561, 18: 1905, 24: 1620 },
    48: { 12: 2254, 18: 1542, 24: 1483 },
    54: { 12: 2030, 18: 1503, 24: 1319 },
  },
  8.4: {
    24: { 12: 5324, 18: 5335, 24: 4701 },
    30: { 12: 4466, 18: 3565, 24: 3445 },
    36: { 12: 2871, 18: 3681, 24: 2531 },
    42: { 12: 3474, 18: 2754, 24: 2710 },
    48: { 12: 3164, 18: 2480, 24: 2287 },
    54: { 12: 3016, 18: 2314, 24: 2379 },
  },
  9.6: {
    24: { 12: 6843, 18: 4070, 24: 3746 },
    30: { 12: 6065, 18: 4732, 24: 3894 },
    36: { 12: 5405, 18: 3880, 24: 3207 },
    42: { 12: 5007, 18: 3541, 24: 3185 },
    48: { 12: 4698, 18: 3287, 24: 3179 },
    54: { 12: 4456, 18: 3093, 24: 2792 },
  },
};
const SNOW_COEFF: Record<string, number> = {
  I: 0.95,
  II: 0.95,
  III: 1.0,
  IV: 1.05,
...
function interpolateRate(
  ...
  for (const h in FRAME_RATES)
    for (const l in FRAME_RATES[+h])
      for (const w in FRAME_RATES[+h][+l])
        points.push({
          length: +l,
          width: +w,
          height: +h,
          rate: FRAME_RATES[+h][+l][+w],
        });
  const withDist = points
    .map((p) => ({
      ...p,
      d: Math.sqrt(
        Math.pow((p.length - length) / 6, 2) +
          Math.pow((p.width - width) / 3, 2) +
          Math.pow((p.height - height) / 1.2, 2),
      ),
    }))
    .sort((a, b) => a.d - b.d);
...
function getCrane5TExtra(
  length: number,
  width: number,
  height: number,
): number {
  const area = length * width;
  const exact = CRANE_5T_DATA.find(
    (p) => p.length === length && p.width === width && p.height === height,
  );
  if (exact) return exact.extra;
  const withDist = CRANE_5T_DATA.map((p) => ({
    ...p,
    d: Math.sqrt(
      Math.pow((p.length - length) / 6, 2) + Math.pow((p.area - area) / 200, 2),
    ),
  })).sort((a, b) => a.d - b.d);
...
function calcSandwichPrice(
  ...
  const baseRate =
    FRAME_RATES[height]?.[length]?.[width] ??
    interpolateRate(length, width, height);
  let frame = area * baseRate;
  frame *= (SNOW_COEFF[snowZone] ?? 1.0) * (WIND_COEFF[windZone] ?? 1.0);
  const craneExtra =
    crane === "Да" && height > 3.6 ? getCrane5TExtra(length, width, height) : 0;
  const base = Math.round(
    roofAndFloor + walls + frame + FIXED_COST + craneExtra,
  );
  return crane === "Да" && height > 3.6 ? Math.round(base * 1.02) : base;
}

function calcProfilePrice(
  length: number,
  width: number,
  height: number,
  snowZone: string,
  ...
  const base = Math.round(
    calcSandwichPrice(length, width, height, snowZone, windZone, crane) /
      1.02 /
      1.23,
  );
...
function quizToPriceParams(
  state: QuizState,
  zones: { snow: string; wind: string },
  length: number,
  width: number,
  height: number,
) {
...
function QuizFullscreen({ onClose }: { onClose: () => void }) {
  ...
  async function sendLead(source: string, extraData?: Record<string, string>) {
    ...
    try {
      await fetch(
        "https://functions.poehali.dev/688d49c8-bb02-4447-8348-7f1bd933e91e",
        {
          method: "POST",
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {STEP1_OPTIONS.map((opt) => (
                  <button
                    key={opt.label}
                    onClick={() =>
                      setState((s) => ({ ...s, purpose: opt.label }))
                    }
              ...
              <div className="grid grid-cols-3 gap-3 md:gap-6 mb-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-1.5">
                    {WIDTHS.map((v) =>
                      dimBtn(v, state.customDims ? -1 : state.width, () =>
                        setState((s) => ({ ...s, width: v, customDims: "" })),
                      ),
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {LENGTHS.map((v) =>
                      dimBtn(v, state.customDims ? -1 : state.length, () =>
                        setState((s) => ({ ...s, length: v, customDims: "" })),
                      ),
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {HEIGHTS.map((v) =>
                      dimBtn(v, state.customDims ? -1 : state.height, () =>
                        setState((s) => ({ ...s, height: v, customDims: "" })),
                      ),
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {submitted ? (
                      ...
                      <>
                        <div
                          className="font-bold text-2xl blur-sm select-none"
                          style={{ color: "var(--orange)" }}
                        >
                          — ₽
                        </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <input
                            ...
                            value={state.phone}
                            onChange={(e) => {
                              const v = phoneMask(e.target.value);
                      ...
                      <label className="flex items-start gap-2.5 cursor-pointer">
                        <div
                          onClick={() =>
                            setState((s) => ({
                              ...s,
                              agreePromo: !s.agreePromo,
                            }))
                          }
                          className="w-5 h-5 rounded mt-0.5 flex items-center justify-center shrink-0 transition-all"
                          style={{
                            background: state.agreePromo
                              ? "var(--orange)"
                              : "transparent",
...
export default function Index() {
  ...
  const [cfTouched, setCfTouched] = useState({
    name: false,
    phone: false,
    email: false,
  });
          ...
          <div className="grid md:grid-cols-[1fr_300px] gap-6 md:gap-12 items-start">
            ...
            >
              ...
              <button
                ...
                className="btn-orange px-6 py-3.5 rounded text-sm inline-flex items-center gap-2"
              >
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                {activeProject.photos.map((_, i) => (
                  <button
                    ...
                    style={{
                      background:
                        modalImgIdx === i ? "#fff" : "rgba(255,255,255,0.4)",
                    }}
                  />
                ))}
              </div>
              <button
                className="absolute right-10 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 flex items-center justify-center shadow"
                onClick={() =>
                ...
                <div className="space-y-2 text-sm">
                  {[
                    ...
                    },
                    {
                      icon: "Ruler",
                      label: "Размеры",
                      value: `${activeProject.width.replace(" м", "")}×${activeProject.length.replace(" м", "")}×${activeProject.height}`,
                    },
                    {
                      icon: "Maximize2",
                      label: "Площадь",
                    ...
                    },
                    {
                      icon: "MapPin",
                      label: "Расположение",
                      value: activeProject.locationShort,
                    },
          ...
          >
            ...
            >
              ...
              <p className="text-white/80 text-sm mt-1">
                Оставьте номер — менеджер свяжется с вами и ответит на все
                вопросы
              </p>
            </div>
